...
class Weights:
    ...
    Args:
        ...
        transforms (Callable): A callable that constructs the preprocessing method (or validation preset transforms)
            needed to use the model_detect. The reason we attach a constructor method rather than an already constructed
            object is because the specific object might have memory and thus we want to delay initialization until
            needed.
        meta (Dict[str, Any]): Stores meta-data related to the weights of the model_detect and its configuration. These can be
            informative attributes (for example the number of parameters/flops, recipe link/methods used in training
            etc), configuration parameters (for example the `num_classes`) needed to construct the model_detect or important
            meta-data (for example the `classes` of a classification model_detect) needed to use the model_detect.
    """
...
class WeightsEnum(Enum):
    """
    This class is the parent class of all model_detect weights. Each model_detect building method receives an optional `weights`
...
def get_model_weights(name: Union[Callable, str]) -> Type[WeightsEnum]:
    """
    Returns the weights enum class associated to the given model_detect.

    Args:
        name (callable or str): The model_detect builder function or the name under which it is registered.

    Returns:
        weights_enum (WeightsEnum): The weights enum class associated with the model_detect.
    """
...
def _get_enum_from_fn(fn: Callable) -> Type[WeightsEnum]:
    """
    Internal method that gets the weight enum of a specific model_detect builder method.

    Args:
        fn (Callable): The builder method used to create the model_detect.
    Returns:
        WeightsEnum: The requested weight enum.
    """
    sig = signature(fn)
...
def get_model_builder(name: str) -> Callable[..., nn.Module]:
    """
    Gets the model_detect name and returns the model_detect builder method.

    Args:
        name (str): The name under which the model_detect is registered.

    Returns:
        fn (Callable): The model_detect builder method.
    """
    name = name.lower()
    try:
        fn = BUILTIN_MODELS[name]
    except KeyError:
        raise ValueError(f"Unknown model_detect {name}")
    return fn


def get_model(name: str, **config: Any) -> nn.Module:
    """
    Gets the model_detect name and configuration and returns an instantiated model_detect.

    Args:
        name (str): The name under which the model_detect is registered.
        **config (Any): parameters passed to the model_detect builder method.

    Returns:
        model_detect (nn.Module): The initialized model_detect.
    """
