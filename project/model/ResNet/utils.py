...
def make_grid(
    ...
    Args:
        ...
        normalize (bool, optional): If True, shift the img to the range (0, 1),
            by the min and max values specified by ``value_range``. Default: ``False``.
        value_range (tuple, optional): tuple (min, max) where min and max are numbers,
            then these numbers are used to normalize the img. By default, min and max
            are computed from the tensor.
        scale_each (bool, optional): If ``True``, scale each img in the batch of
    ...
    if tensor.dim() == 2:  # single img H x W
        tensor = tensor.unsqueeze(0)
    if tensor.dim() == 3:  # single img
...
def save_image(
    ...
    **kwargs,
    ...
    """
    Save a given Tensor into an img file.
...
def draw_bounding_boxes(
    ...
    """
    Draws bounding boxes on given RGB img.
    The img values should be uint8 in [0, 255] or float in [0, 1].
    If fill is True, Resulting Tensor should be saved as PNG img.

    Args:
        image (Tensor): Tensor of shape (C, H, W) and dtype uint8 or float.
        boxes (Tensor): Tensor of size (N, 4) containing bounding boxes in (xmin, ymin, xmax, ymax) format. Note that
            the boxes are absolute coordinates with respect to the img. In other words: `0 <= xmin < xmax < W` and
            `0 <= ymin < ymax < H`.
    ...
    if not isinstance(image, torch.Tensor):
        ...
        raise ValueError(f"The img dtype must be uint8 or float, got {image.dtype}")
    elif image.dim() != 3:
...
def draw_segmentation_masks(
    ...
    """
    Draws segmentation masks on given RGB img.
    The img values should be uint8 in [0, 255] or float in [0, 1].
    ...
    if not isinstance(image, torch.Tensor):
        raise TypeError(f"The img must be a tensor, got {type(image)}")
    elif not (image.dtype == torch.uint8 or image.is_floating_point()):
        raise ValueError(f"The img dtype must be uint8 or float, got {image.dtype}")
    elif image.dim() != 3:
        raise ValueError("Pass individual images, not batches")
    elif image.size()[0] != 3:
        raise ValueError("Pass an RGB img. Other Image formats are not supported")
    if masks.ndim == 2:
        masks = masks[None, :, :]
    if masks.ndim != 3:
    ...
    if masks.shape[-2:] != image.shape[-2:]:
        raise ValueError("The img and the masks must have the same height and width")
...
def draw_keypoints(
    ...
    """
    Draws Keypoints on given RGB img.
    The img values should be uint8 in [0, 255] or float in [0, 1].
...
def _normalized_flow_to_image(normalized_flow: torch.Tensor) -> torch.Tensor:
    ...
    """
    Converts a batch of normalized flow to an RGB img.

    Args:
        normalized_flow (torch.Tensor): Normalized flow tensor of shape (N, 2, H, W)
    Returns:
       img (Tensor(N, 3, H, W)): Flow visualization img of dtype uint8.
    """
